...
describe 'Container(CS GO) API', :type => :request do
  ...
  it 'Allows to start container' do
    ...
    container = Containers::CounterStrikeGo.find(@context.container_id) rescue nil
    expect(container).not_to be_nil
    
    ap "Port is #{container.port}"

    app_id = Containers::CounterStrikeGo::STEAM_APP_ID
    gslt = container.config.get_property_value("gslt")
    
    gslt_is_valid = SteamServerLoginToken.exists?(app_id: app_id, token: gslt, in_use: true)
  ...
  it 'Allows to destroy container' do
    ...
    gslt_is_valid = SteamServerLoginToken.exists?(app_id: app_id, token: gslt, in_use: false)
    expect(gslt_is_valid).to be(true)
  end
